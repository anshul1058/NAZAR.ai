...
class _ChildDetailScreenState extends State<ChildDetailScreen> {
  ...
  // Realtime listener — auto-refresh when the child's status changes
    ...
    _statusChannel = Supabase.instance.client
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            ...
            AppLogger.d('Detail — status changed in realtime!');
            setState(() {
  ...
  // Fetch fresh connection status from the DB
  Future<void> _refreshChildStatus() async {
    try {
      ...
      AppLogger.d('Failed to refresh child status → $e');
    }
  }
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              "Scan this QR from the child's phone to\nreconnect them",
              style: TextStyle(
                fontSize: 13,
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      "Open PERISAI on the child's phone → pick \"I'm a Child\" → Scan this QR",
                      style: TextStyle(
                        fontSize: 12,
                        color: AppColors.textSecondary,
                        height: 1.4,
  ...
  Future<void> _showEditSheet() async {
    ...
    DateTime selectedDate = DateTime(now.year - _child.age, now.month, now.day);
    ...
    await showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      builder: (sheetCtx) {
        // Take MediaQuery from the parent context (not the sheet context)
        // to avoid the _dependents.isEmpty crash
        final screenHeight = MediaQuery.of(context).size.height;
        return StatefulBuilder(
          builder: (innerCtx, setModal) {
            // Compute age from the selected date
                    ...
                    child: Row(
                      children: [
                        ...
                        const Column(
                          ...
                          children: [
                            Text('Edit Child Details',
                                style: TextStyle(
                                    fontSize: 16,
                                    fontWeight: FontWeight.w700,
                                    color: AppColors.textPrimary)),
                            Text("Update your child's profile",
                                style: TextStyle(
                                    fontSize: 12,
                                    color: AppColors.textSecondary)),
                          ],
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            TextFormField(
                              ...
                              decoration: _inputDecoration("Enter the child's name",
                            ...
                            const Text("Child's Phone Number",
                                style: TextStyle(
                                    fontSize: 13,
                                    fontWeight: FontWeight.w600,
                                    color: AppColors.textPrimary)),
                            ...
                            TextFormField(
                              ...
                              decoration: _inputDecoration(
                                  'Example: 08123456789', Icons.phone_outlined),
                            ),
                            const SizedBox(height: 16),
                              ...
                              child: Container(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Expanded(
                                      child: Text(
                                        '${selectedDate.day}/${selectedDate.month}/${selectedDate.year}  •  $computedAge years old',
                                        style: const TextStyle(
                                            fontSize: 14,
                                            color: AppColors.textPrimary),
                                      ),
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: saving
                                    ...
                                    : () async {
                                        ...
                                        try {
                                          // Compute the final age from the date of birth
                                          int newAge =
                                          ...
                                          // Upload the photo if one was picked
                                          String? newAvatarUrl =
                                              _child.avatarUrl;
                                          ...
                                          ScaffoldMessenger.of(context)
                                            ...
                                            SnackBar(
                                              content: const Text(
                                                  "Child's details updated successfully ✅"),
                                              backgroundColor:
                                                  AppColors.success,
                                              behavior:
  ...
  Future<void> _togglePause() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(
          isOnline ? 'Pause Monitoring?' : 'Reconnect?',
          style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 17),
        ),
        content: Text(
          isOnline
              ? "Monitoring of ${_child.firstName}'s phone will be paused. PERISAI won't detect gambling content until it's reconnected."
              : "Resume monitoring ${_child.firstName}'s phone?",
          style: const TextStyle(
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: isOnline ? AppColors.warning : AppColors.primary,
            ...
            child: Text(isOnline ? 'Pause' : 'Reconnect'),
          ),
        ],
      ),
    );
    ...
    try {
      // Parent (authenticated) UPDATEs directly — RLS allows updating own children.
      // Pause: set paused_until far into the future (indefinite until resumed) +
      //        status offline_manual. The heartbeat RPC will respect paused_until.
      // Resume: paused_until null + online (the child's heartbeat keeps it fresh).
      if (isOnline) {
        // "Indefinite" sentinel — pause until the parent reconnects
        final pausedUntil = DateTime.utc(2999);
        await Supabase.instance.client.from('children').update({
        ...
        setState(() {
          _child = _child.copyWith(
            connectionStatus: ConnectionStatus.offlineManual,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isOnline
              ? 'Monitoring paused for now ⏸️'
              : 'Monitoring re-enabled ✅'),
          backgroundColor: isOnline ? AppColors.warning : AppColors.success,
                        ...
                        delegate: SliverChildBuilderDelegate(
                          (context, i) => Padding(
                            ...
                            child: _DetectionTile(
                              ...
                              title:
                                  _detectionTitle(_detections[i].triggeredBy),
        ...
        children: [
          ...
          Text('No detections',
              style: TextStyle(
                  ...
                  color: AppColors.textPrimary)),
          SizedBox(height: 4),
          Text("The child's phone is safe from harmful content",
