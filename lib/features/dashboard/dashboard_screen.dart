...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  // Auto-refresh when the child's connection status changes in the DB
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            ...
            if (childIds.contains(newData['id'])) {
              AppLogger.d('Child status changed → reload dashboard');
              _loadData();
            }
          },
        )
  ...
  Future<void> _showLocalNotif(Map<String, dynamic> data) async {
    try {
      ...
      final label = switch (by) {
        'ocr' => 'Read Text',
        'mobilenet' => 'View Image',
        'trustpositif' => 'Check URL',
        'combined' => 'Combined',
        _ => by,
      };
      await _localNotif.show(
        ...
        'AI is $pct% confident — detected via $label',
        const NotificationDetails(
          android: AndroidNotificationDetails(
            'nazar_ai_channel',
            'NAZAR.AI Detection',
            channelDescription: 'Gambling detection notifications from NAZAR.AI',
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        _userName = user.userMetadata?['full_name'] ?? 'Parent';
      });

      // Fetch children with avatar_url
  ...
  void _showLogoutDialog() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Ready to log out? 👋',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Logout only — settings is already in the navbar
          IconButton(
            icon:
      ...
      // FAB removed — the + button is already in the navbar
    );
  }
}
...
class _ChildAvatar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Border color based on connection status
        ...
        children: [
          Stack(
            ...
            children: [
              ...
              // Status indicator — dynamic based on connection status
              Positioned(
                top: 0,
                left: 0,
...
/// Small connection indicator widget — reusable
...
class _ActivityCard extends StatelessWidget {
  ...
  String get _desc {
    if (detection.keywords.isNotEmpty) {
      return 'Suspicious words: ${detection.keywords.join(', ')}';
    }
    return 'Detected via ${detection.triggeredByLabel} '
        '— ${detection.confidencePercent} confident';
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // Child avatar with danger badge
            Stack(
              clipBehavior: Clip.none,
              children: [
                Container(
